...
public class Day5Part2 implements Solution {
    public static void main(String[] args) {
        System.out.println(new Day5Part2().solve(Utils.readFile("day5.txt")));
    }
        ...
        public void addLine(Pair<Integer, Integer> from, Pair<Integer, Integer> to) {
            if (Objects.equals(from.getT(), to.getT())) { //vertical
                ...
                if (from.getT() > to.getT()) {
                    Pair<Integer, Integer> swap = from;
                    from = to;
                    to = swap;
                }
                int yAdj = 0;
                int yGrowth = to.getU() > from.getU() ? 1 : -1;
                for (int x = from.getT(); x <= to.getT(); x++) {
                    putAtPoint(x, from.getU() + yAdj);
                    yAdj += yGrowth;
                }
            }
        }
